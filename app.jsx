...
// ━━━ 노션 실시간 데이터 (마지막 동기화: {lastSync || '로딩 중...'}) ━━━
...
function App() {
  ...
  // 노션 실시간 데이터
  const [NOTION_ENTRIES, setNotionEntries] = useState(NOTION_ENTRIES_DEFAULT);
  const [NOTION_집계, setNotion집계] = useState(NOTION_집계_DEFAULT);
  ...
  const fetchNotion = useCallback(async () => {
    ...
    try {
      ...
      if (data.entries) setNotionEntries(data.entries);
      if (data.집계) setNotion집계(data.집계);
  ...
  useEffect(() => { fetchNotion(); }, [fetchNotion]);
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Noto Sans KR',sans-serif",color:C.text,paddingBottom:74}}>
      ...
      <div style={{padding:"14px",maxWidth:460,margin:"0 auto"}}>
        ...
        {tab==="home" && <>
          {/* 동기화 버튼 */}
          <button onClick={fetchNotion} disabled={notionLoading} style={{
            width:"100%",marginBottom:10,
            background:notionLoading?"#1a3050":C.blue2,
            border:`1px solid ${C.blue}`,borderRadius:10,
            color:C.blue,padding:"10px",fontSize:13,
            fontWeight:700,cursor:notionLoading?"not-allowed":"pointer",
          }}>
            {notionLoading ? "⏳ 노션 동기화 중..." : "🔄 노션 데이터 새로고침"}
          </button>
          ...
          <Card>
            ...
            <div style={{marginTop:12,borderTop:`1px solid ${C.border}`,paddingTop:12}}>
              <div style={{fontSize:11,color:C.dim,marginBottom:8,fontWeight:600}}>
                📋 전체 내역 ({NOTION_ENTRIES.length}건) · 마지막 동기화: {lastSync || '로딩 중...'}
              </div>
